      ...
      const sendData = async () => {
          ...
          try {
              ...
              if (response.ok) {
                  ...
                  // Como último recurso, usar pixel GET
                  this.sendPixel(data);
              }
          } catch (error) {
              ...
              // Muchos bloqueadores lanzan ERR_BLOCKED_BY_CLIENT. En ese caso usar pixel GET
              this.sendPixel(data);
          }
      };

      // Envío mediante pixel (imagen 1x1) usando GET y query params
      this.sendPixel = (payload) => {
          try {
              const d = Number.isFinite(payload.duration) ? payload.duration : 0;
              const u = encodeURIComponent(payload.url || window.location.href);
              const cacheBust = Date.now();
              const img = new Image(1, 1);
              // Algunos navegadores permiten controlar el referer
              try { img.referrerPolicy = 'no-referrer'; } catch {}
              const src = `${this.pixelEndpoint}?u=${u}&d=${d}&_=${cacheBust}`;
              img.src = src;
              if (this.debug) console.log('→ Pixel enviado:', src);
          } catch (e) {
              if (this.debug) console.error('Fallo al enviar pixel:', e);
          }
      };
...
if (typeof window !== 'undefined') {
    ...
    console.log('Analytics performance cargado desde:', window.location.origin);
}
